...
Stacks can be built using arrays and linkedlists. In this blog we will be using an array to build a stack.
...
Follwoing code can be used to address the exception.
...
```
To remove the top element from the stack, decrement count (index of top element) by 1 to make the count point the next element as the top element.
