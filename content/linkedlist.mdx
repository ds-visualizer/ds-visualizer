...
LinkedList is a data structure which is similar to an array but each element are is pointed to the next element but a pointer.
...
> Since each Node is a custom data type and that is not know at the compile time, they are stored in the heap memory, **So remember to free the memory when you are done with it, high level languages have an inbuilt garbage collector so no need of worrying about it**
...
// Suppose the there is no linked list, the variable will be holding null.
...
When calling this method, we will iterate till the last second node and change the `next` value to null so it will stop holding the position of the previous last
