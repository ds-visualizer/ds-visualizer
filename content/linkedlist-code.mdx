...
Generally LinkedList are done in `2 ways`, one way by using `2 pointers` where one points `Head` and `Tail` node.

In this blog we will use 2 pointers where one holds fist node and other holds tail node.
...
Basically says if head isn't holding any node that means `there is no linked list available`, then we make head and tail refer to the new Node.
...
This looks similar too isn't? we already have a variable thats pointing to the last node, so all we do now is make `tail point to the new Node` and
...
```java:insertAt.java
	...
	public void insertAt(int index, int value) {
		...
		*/
		...
		// Make a temporary variable such that it holds the other nodes thats after 1
...
**Now with all the information you read, try to build contains method. Thats your homework 😉**
