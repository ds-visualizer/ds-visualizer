...
Generally LinkedList are done in `2 ways`, one way by using `2 pointers` where one points `first` and `last` node.
...
```java:get.java
	...
	public int get(int index){
		...
		for(int i = 0; i < index; i++) { // iterate till that node and return the node
			current = current.next;
		}
