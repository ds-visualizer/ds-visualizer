...
Generally LinkedLists are done in `2 ways`, one way by using `2 pointers` where one points `Head` and `Tail` node and other way is just using a pointer `root` to hold the head pointer.
